...
contract DividendsV2 is Ownable, ReentrancyGuard, IXFlashTokenUsage, IDividendsV2 {
    ...
    function allocate(
        ...
        uint256 amount,
        bytes calldata /*data*/
    ) external override nonReentrant xFlashTokenOnly {
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userXFlashAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userXFlashAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXFlashAllocation.mul(accDividendsPerShare).div(1e18);
